...
object DateUtils {
    ...
    fun parseIsoDate(value: String): LocalDate = try {
        if (!Regex("^\\d{4}-\\d{2}-\\d{2}$").matches(value)) throw IllegalArgumentException("Invalid date: $value")
        LocalDate.parse(value, iso)
    } catch (_: DateTimeParseException) { throw IllegalArgumentException("Invalid date: $value") }
    ...
    fun isValidTimeZone(timeZone: String): Boolean = try { ZoneId.of(timeZone); timeZone.isNotEmpty() && timeZone.length <= 255 } catch (_: Exception) { false }
    ...
    private fun zone(value: String): ZoneId { if (!isValidTimeZone(value)) throw IllegalArgumentException("Invalid IANA time zone."); return ZoneId.of(value) }
}
