...
The adapter-node image runs on port `4100` and expects PostgreSQL through `DATABASE_URL`. Production Compose uses the same image for web, worker, and migration roles. Migrations complete before web/worker cutover.
